...
function Stat({ label, value, accent }: { label: string; value: number; accent?: boolean }) {
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <p className={`text-2xl font-bold tabular-nums ${accent ? "text-desyn-success" : "text-foreground"}`}>{value.toLocaleString()}</p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <button onClick={runExecute} disabled={loading !== null || !preview || preview.reclaimed_applies === 0}
          className="inline-flex items-center gap-2 rounded-lg bg-destructive px-4 py-2 text-sm font-semibold text-white hover:opacity-90 disabled:opacity-40">
      ...
      {preview && !result && (
        <div>
          ...
          {preview.reclaimed_applies > 0
            ? <p className="mt-3 text-sm text-amber-600 dark:text-amber-400">Executing will re-charge these {preview.users_affected} user(s) {preview.reclaimed_credits.toLocaleString()} credits total.</p>
      ...
      {result && (
        <div>
          <p className="mb-2 flex items-center gap-1.5 text-sm font-semibold text-desyn-success"><CheckCircle2 className="h-4 w-4" /> Reclaim executed</p>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">

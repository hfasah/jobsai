...
export default function AdminUserDetail({ params }: { params: Promise<{ userId: string }> }) {
  ...
  // "Open account" — mint a Clerk actor token server-side, then sign in as the
  // user via the ticket strategy. The session carries the admin as `actor`.
  const openAccount = async () => {
    setOpening(true); setActionMsg(null);
    try {
      const res = await fetch(`/api/admin/users/${userId}/impersonate`, { method: "POST" });
      const json = await res.json();
      if (!res.ok || !json.ticket) { setActionMsg(json.error ?? "Could not open account."); return; }
      const signIn = await clerk.client.signIn.create({ strategy: "ticket", ticket: json.ticket });
        ...
        <div className="flex flex-col items-stretch gap-2 shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button onClick={openAccount} disabled={opening || banned}
              title={banned ? "Reactivate the account first" : "Sign in as this user"}

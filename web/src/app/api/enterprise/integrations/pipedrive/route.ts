...
export async function GET() {
  ...
  return NextResponse.json({
    data: {
      ...
      deals: counts.deals,
      syncedDeals: counts.syncedDeals,
    },
  });
}
...
export async function POST(req: NextRequest) {
  ...
  const counts = await pipedriveSyncCounts(g.org.id);
  return NextResponse.json({ data: { connected: true, domain, company_name: companyName, last_sync: null, companies: counts.companies, synced: counts.synced, contacts: counts.contacts, syncedContacts: counts.syncedContacts, deals: counts.deals, syncedDeals: counts.syncedDeals } }, { status: 201 });
}

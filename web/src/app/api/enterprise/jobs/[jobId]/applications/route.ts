...
// Public-facing: submit an application (no auth needed)
export async function POST(req: NextRequest, { params }: { params: Promise<{ jobId: string }> }) {
  ...
  const { data: existing } = await supabaseAdmin
    ...
    .select("id")
    .eq("job_id", jobId)
    .eq("candidate_email", body.candidate_email.trim().toLowerCase())
    .maybeSingle();
  ...
  await resend.emails.send({
    ...
    html: `<div style="font-family:sans-serif;max-width:560px;margin:0 auto">
      ...
      <p>Thank you for applying for <strong>${job.title}</strong>. We've received your application and will be in touch soon.</p>

    ...
    articles: [
      ...
      },
      {
        ...
        sections: [
          {
            body: [
              "If your team runs a separate sales CRM, JobsAI can push your recruiting CRM — companies and contacts — into it so both teams work from one set of records and nobody double-enters. Pipedrive is live today; HubSpot, Salesforce, Zoho and others are available on request.",
              "Sync is one-way (JobsAI → your CRM) and deduplicated: each JobsAI record is keyed to its CRM object, so updates modify the same record instead of creating duplicates, and nothing in your CRM is deleted. Records push automatically in the background, and a Sync now button pushes everything at once.",
            ],
          },
          {
          ...
          },
          {
            ...
            body: [
              "“Couldn't connect to Pipedrive” — the API token is wrong or expired. Re-copy it from Pipedrive → Settings → Personal preferences → API.",
              "Records aren't appearing — confirm the integration shows Connected, then click Sync now, and check you're an Owner/Admin.",
              "A contact isn't under its company — make sure the contact has a company set in JobsAI; contacts without one sync as standalone Persons.",
            ],
          },
          {
      ...
      },
      {
        ...
        sections: [
          ...
          },
          {
            ...
            steps: [
              "Open Settings → Intake to see your address; customise the handle and Save (e.g. recrutamento@talent.jobsai.work), then Copy it.",
              ...
              "Confirm the forwarding: Google/Microsoft sends a one-time confirmation request to the intake address. JobsAI catches it and shows the confirmation code plus a “Confirm in Gmail” link as a banner on Settings → Intake — enter that code (or click the link) in your mailbox's forwarding settings to activate it. Tip: an admin-created forwarding rule or alias usually skips this confirmation step entirely.",
          ...
          },
          {
            ...
            steps: [
              "Set your Reply-to address under Settings → Intake (available on every plan). Candidate replies to interview invites, offers, and reminders then land in that monitored inbox. Leave it blank to fall back to your org contact email.",
              "With a connected mailbox, replies also come straight back to your own inbox — read and respond from there, and the thread stays under your address.",
            ...
            tip: "Connected Google mailboxes currently use send + calendar access only, so replies arrive in your mailbox (or your Reply-to inbox) where you read and respond — a threaded reply view inside JobsAI isn't available with send-only access. Always point Reply-to at a mailbox someone watches.",
          },
          {
            heading: "Recommended setup",
            steps: [
              "Receive: forward careers@yourcompany.com → your talent.jobsai.work intake address, then confirm forwarding via the banner on Settings → Intake.",
              "Send: connect your Gmail/Microsoft mailbox (or enable white-label sending on a supported plan).",
              "Reply: set your Reply-to under Settings → Intake (every plan) so replies reach a monitored mailbox; forward any stray candidate emails to intake to keep everything filed.",
            ],

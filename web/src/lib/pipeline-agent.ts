...
interface Rule {
  ...
  // v1 single action (legacy)
  action: string;
  action_config: Record<string, unknown>;
  // v2 multi-action (new) — takes precedence when set
...
function evaluateCondition(cond: Condition, app: AgentApplication): boolean {
  ...
  switch (cond.operator) {
    ...
    case "is_empty":  return !raw || (Array.isArray(raw) && raw.length === 0);
    case "not_empty": return !!raw && !(Array.isArray(raw) && raw.length === 0);
...
async function executeSingleAction(
  ...
  jobTitle: string,
): Promise<{ result: string; details: Record<string, unknown> }> {
  switch (action) {
    case "move_stage": {
      ...
      if (ORDER.indexOf(stage) <= ORDER.indexOf(app.stage)) {
        return { result: "skipped", details: { reason: "Already at or past this stage" } };
      }
      await supabaseAdmin
        .from("enterprise_applications")
        .update({ stage, stage_updated_at: new Date().toISOString() })
        .eq("id", app.id);
      app.stage = stage; // mutate in-place so subsequent actions see updated stage
      ...
      if (cfg.send_email && app.candidate_email) {
        ...
        await resend.emails.send({
          ...
          html: `<div style="font-family:sans-serif;max-width:560px;color:#0f172a">
            ...
            <p>Thank you for applying for <strong>${jobTitle}</strong>. After careful review, we've decided to move forward with other candidates at this time.</p>
    ...
    case "notify_hm": {
      ...
      await resend.emails.send({
        ...
        html: `<div style="font-family:sans-serif;max-width:560px;color:#0f172a">
          ...
          <p>Hi,</p>
          <p>The recruiting agent flagged a candidate for your review:</p>
          <table style="width:100%;border-collapse:collapse;margin:12px 0">
            <tr><td style="padding:4px 0;color:#64748b;font-size:13px">Candidate</td><td style="font-weight:600">${app.candidate_name}</td></tr>
            <tr><td style="padding:4px 0;color:#64748b;font-size:13px">Role</td><td>${jobTitle}</td></tr>
            <tr><td style="padding:4px 0;color:#64748b;font-size:13px">Match score</td><td style="font-weight:600;color:#16a34a">${app.match_score ?? "—"}%</td></tr>
            <tr><td style="padding:4px 0;color:#64748b;font-size:13px">AI recommendation</td><td>${app.ai_recommendation ?? "—"}</td></tr>
          </table>
    ...
    case "send_interview_invite": {
      ...
      if (!inviteData?.token) {
        return { result: "skipped", details: { reason: "No interview token — create an AI interview for this job first" } };
      }
      const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? "https://jobsai.work";
      const orgRes = await supabaseAdmin.from("enterprise_orgs").select("name").eq("id", app.org_id).maybeSingle();
      const orgName = orgRes.data?.name ?? "The hiring team";
      ...
      await resend.emails.send({
        ...
        html: `<div style="font-family:sans-serif;max-width:560px;color:#0f172a">
          <p>Hi ${app.candidate_name},</p>
          <p>We were impressed with your application for <strong>${jobTitle}</strong> and would like to invite you to a short AI-powered interview (~15 min, complete any time).</p>
    ...
    case "assign_to_recruiter": {
      const recruiterId = cfg.recruiter_id as string;
      if (!recruiterId) return { result: "error", details: { reason: "No recruiter configured" } };
      await supabaseAdmin
        .from("enterprise_applications")
        .update({ assigned_to: recruiterId })
        .eq("id", app.id);
      return { result: "success", details: { assigned_to: recruiterId } };
    }

    default:
      return { result: "error", details: { reason: `Unknown action: ${action}` } };
  }
}

// ── Run all actions for a rule ────────────────────────────────────────────────
...
async function executeRule(
  ...
  // v2: multi-action array
  const steps: ActionStep[] = Array.isArray(rule.actions) && rule.actions.length > 0
    ? rule.actions
  ...
  for (const step of steps) {
    if (firedActions.has(step.action)) continue; // dedupe within run
    const r = await executeSingleAction(step.action, step.action_config, app, jobTitle);
    stepResults.push({ action: step.action, ...r });
    if (r.result === "success") firedActions.add(step.action);
    if (r.result === "error") overallResult = "error";
  }

  return { result: overallResult, details: { steps: stepResults } };
}
...
export async function runPipelineAgent(
  app: AgentApplication,
  jobTitle: string,
  triggerEvent = "application_screened",
): Promise<void> {
  ...
  for (const rule of rules) {
    ...
    const { result, details } = await executeRule(rule as Rule, app, jobTitle, firedActions);

    void supabaseAdmin.from("enterprise_agent_actions").insert({
      org_id: app.org_id,
      rule_id: rule.id,
      rule_name: rule.name,
...
// ── Stale candidate agent (cron) ──────────────────────────────────────────────

export async function runStaleAgent(orgId: string): Promise<number> {
  const { data: rules } = await supabaseAdmin
    .from("enterprise_pipeline_rules")
    .select("*")
    .eq("org_id", orgId)
  ...
  for (const rule of rules) {
    const staleDays = (rule.trigger_config as Record<string, unknown>)?.stale_for_days as number ?? 7;
    const cutoff = new Date(Date.now() - staleDays * 86_400_000).toISOString();

    let query = supabaseAdmin
      .from("enterprise_applications")
      .select("id,job_id,candidate_name,candidate_email,stage,match_score,ats_score,ai_recommendation,risk_flags,ats_keywords_matched,ats_keywords_missing")
      .eq("org_id", orgId)
      .not("stage", "in", '("hired","rejected")')
      .lte("stage_updated_at", cutoff);

    if (rule.job_id) query = query.eq("job_id", rule.job_id);

    const { data: staleApps } = await query;
    if (!staleApps?.length) continue;
    ...
    for (const appRow of staleApps) {
      const app: AgentApplication = {
        ...
        candidate_name: appRow.candidate_name,
        candidate_email: appRow.candidate_email,
        stage: appRow.stage,
        match_score: appRow.match_score,
        ats_score: appRow.ats_score,
        ai_recommendation: appRow.ai_recommendation,
        risk_flags: appRow.risk_flags,
        ats_keywords_matched: appRow.ats_keywords_matched,
        ats_keywords_missing: appRow.ats_keywords_missing,
      };

      // Additional conditions (beyond the stale check itself) must also match
      const extraConditions = (rule.conditions as Condition[]).filter((c) => c.field !== "stale_for_days");
      if (extraConditions.length > 0) {
        const passes = extraConditions.every((c) => evaluateCondition(c, app));
        if (!passes) continue;
      }

      const { data: jobRow } = await supabaseAdmin
        .from("enterprise_jobs").select("title").eq("id", app.job_id).maybeSingle();
    ...
    void supabaseAdmin
      .from("enterprise_pipeline_rules")
      .update({ run_count: (rule.run_count ?? 0) + fired, last_run_at: new Date().toISOString() })
      .eq("id", rule.id);
  }

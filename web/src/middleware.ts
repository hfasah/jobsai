...
// Resolve a custom domain to an org slug via Supabase REST (edge-compatible)
...
export default clerkMiddleware(async (auth, req: NextRequest) => {
  ...
  // Skip custom-domain routing for the main domain, localhost, and Vercel previews
  const isKnownHost =
    hostname === APP_HOST ||
  ...
  if (!isKnownHost && hostname) {
    ...
    if (slug) {
      // Rewrite: careers.acme.com/anything → /careers/slug/anything

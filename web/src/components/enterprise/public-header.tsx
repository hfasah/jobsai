        ...
        <nav className="hidden items-center gap-1 text-sm md:flex">
          ...
          <div className="group relative">
            ...
            <div className="invisible absolute left-1/2 top-full z-50 -translate-x-1/2 pt-2 opacity-0 transition-all duration-150 group-hover:visible group-hover:opacity-100">
              <div className="w-[640px] rounded-2xl border border-border bg-card p-4 shadow-xl">
                <p className="px-2 pb-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Built for</p>
                <div className="grid grid-cols-2 gap-1">
                  {PERSONAS.map((p) => (
                    <Link
                      key={p.slug}
                      href={`/enterprise/built-for/${p.slug}`}
                      className="group/item flex items-start justify-between gap-3 rounded-xl px-3 py-2.5 hover:bg-muted"
                    >
                ...
                <Link
                  href="/enterprise/built-for"
                  className="mt-1 flex items-center justify-center rounded-xl border border-border px-3 py-2 text-xs font-semibold text-foreground hover:bg-muted"
                >
                  See all use cases
                </Link>
              </div>
